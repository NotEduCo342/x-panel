...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/vaxilu/x-panel/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
check_config() {
    info=$(/usr/local/x-panel/x-panel setting -show true)
    if [[ $? != 0 ]]; then
...
enable() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "x-panel 设置开机自启成功"
    else
        LOGE "x-panel 设置开机自启失败"
    fi
...
disable() {
    systemctl disable x-panel
    if [[ $? == 0 ]]; then
        LOGI "x-panel 取消开机自启成功"
    else
        LOGE "x-panel 取消开机自启失败"
    fi
...
update_shell() {
    wget -O /usr/bin/x-panel -N --no-check-certificate https://github.com/vaxilu/x-panel/raw/master/x-panel.sh
    if [[ $? != 0 ]]; then
        echo ""
...
————————————————
  ${green}8.${plain} 启动 x-panel
  ${green}9.${plain} 停止 x-panel
  ${green}10.${plain} 重启 x-panel
  ${green}11.${plain} 查看 x-panel 状态
  ${green}12.${plain} 查看 x-panel 日志
————————————————
  ${green}13.${plain} 设置 x-panel 开机自启
  ${green}14.${plain} 取消 x-panel 开机自启
————————————————
