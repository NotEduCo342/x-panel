...
#This function will be called when user installed x-panel out of sercurity
config_after_install() {
    ...
    if [[ x"${config_confirm}" == x"y" || x"${config_confirm}" == x"Y" ]]; then
        ...
        /usr/local/x-panel/x-panel setting -username ${config_account} -password ${config_password}
        echo -e "${yellow}账户密码设定完成${plain}"
        /usr/local/x-panel/x-panel setting -port ${config_port}
...
install_x-panel() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/vaxilu/x-panel/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/x-panel-linux-${arch}.tar.gz https://github.com/vaxilu/x-panel/releases/download/${last_version}/x-panel-linux-${arch}.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/vaxilu/x-panel/releases/download/${last_version}/x-panel-linux-${arch}.tar.gz"
    ...
    cd x-panel
    chmod +x x-panel bin/xray-linux-${arch}
    cp -f x-panel.service /etc/systemd/system/
    wget --no-check-certificate -O /usr/bin/x-panel https://raw.githubusercontent.com/vaxilu/x-panel/main/x-panel.sh
    chmod +x /usr/local/x-panel/x-panel.sh
